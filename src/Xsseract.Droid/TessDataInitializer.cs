...
namespace Xsseract.Droid
{
  public class TessDataInitializer
  {
    ...
    {
      await Task.Factory.StartNew (DoInitialize);
    }
    ...
    private void DoInitialize ()
    {
      ...
      bool shouldRefreshDataFiles = tessDataFiles.Count == 0 || !IsFileCollectionSane (tessDataFiles);
      if (shouldRefreshDataFiles) {
        context.LogWarn ("Some tess data files are missing. Redeploying ...");
        RemoveFiles (tessDataFiles);
      }
      Android.Util.Log.Debug ("Xsseract.App", "@3");
      bool shouldRefreshOrientationFiles = tessOrientationFiles.Count == 0 || !IsFileCollectionSane (tessOrientationFiles);
      ...
      if (null != dstFiles) {
        foreach (var f in dstFiles) {
          if (!tessDataFiles.Contains (f, feq) && !tessOrientationFiles.Contains (f, feq)) {
    ...
    {
      return String.Format ("{0}.download", fileName);
    }
    ...
    {
      return String.Format ("{0}.src", fileName);
    }
